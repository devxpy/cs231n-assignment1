...
#         print(X.shape, W1.shape, b1.shape, h1.shape, W2.shape, b2.shape, scores.shape)
        ...
        cls_idx = np.arange(N), y
        
        scores -= np.max(scores, axis=1)[:, None]
        e = np.exp(scores)
        f = e / np.sum(e, axis=1)[:, None]
        losses = -np.log(f[cls_idx])
        loss = np.average(losses) + reg * (np.sum(W1 ** 2) + np.sum(W2 ** 2))
